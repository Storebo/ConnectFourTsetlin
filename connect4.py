...
def StartMachine(clauses,epoch,Threshold,S, inndata):
    timestr = stime.strftime("%m%d-%H%M")
    results = open("2Dresults/2D"+timestr+".csv",'a')
    resultclauses = open("2Dresults/2D"+timestr+"clauses1.csv",'a')
    ...
    ecount = 0
    counter = 0
    results.write("MultiClassTsetlinMachine,2D,")
    while ecount < epoch:
    ...
    while counter < 10:
        ...
        print("-------------------------------------------------------------------------------------------")
        print("MultiClassTsetlinMachine using %s%s writen to file %s.csv\n" %(inndata,numb,timestr))
        ...
        if(counter == 10):
            ...
            resultclauses.close()
            resultclauses = open("2Dresults/2D" + timestr + "clauses0.csv", 'a')
            PrintClass(tm, 0, clauses)
            resultclauses.close()
            if (inndata == "draw"):
                resultclauses = open("2Dresults/2D" + timestr + "clauses2.csv", 'a')
...
def runner():
    ...
    #StartMachine(clauses,epoch,Threshold,S,inndata)
    StartMachine(clauses, epoch, Threshold, 20.0, "nodraw")
