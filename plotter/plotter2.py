...
CI = True
table = open("table",'w')
for FILENAME in ["7x6 10000c 90t without draw","4x4 10000c 90t without draw", "4x4 10000c 90t with draw"]:
    m = []
    s = []
    c = 0
    counter =0
    ...
    with open(f"{FILENAME}", 'r') as file:
        ...
        table.write(backslash+"begin{tabular}[c]{@{}l@{}} "+window+"\end{tabular}\n")
        ...
        table.write("&"+backslash+"begin{tabular}[c]{@{}l@{}}"+str(round(m[14],2))+backslash+"%$"+backslash+"pm"+str(round(s[14],2))+backslash+"%$"+backslash+"end{tabular}\n")
        table.write("&"+backslash+"begin{tabular}[c]{@{}l@{}}"+str(round(m[29],2))+backslash+"%$"+backslash+"pm"+str(round(s[29],2))+backslash+"%$"+backslash+"end{tabular}\n")
        table.write("&"+backslash+"begin{tabular}[c]{@{}l@{}}"+str(round(m[49],2))+backslash+"%$"+backslash+"pm"+str(round(s[49],2))+backslash+"%$"+backslash+"end{tabular}"+backslash+backslash+" "+backslash+"hline\n")
...
for FILENAME in ["7x6 10000c 90t without draw","4x4 10000c 90t without draw", "4x4 10000c 90t with draw"]:
    ...
    with open(f"{FILENAME}", 'r') as file:
        ...
        for i in range(len(line)):
            m.append(median(
                [tab[0][i], tab[1][i], tab[2][i], tab[3][i], tab[4][i], tab[5][i], tab[6][i], tab[7][i], tab[8][i],
                 tab[9][i]]))
            s.append(stdev(
                [tab[0][i], tab[1][i], tab[2][i], tab[3][i], tab[4][i], tab[5][i], tab[6][i], tab[7][i], tab[8][i],
                 tab[9][i]]))
    ...
    pyplot.plot(x[OFFSET:], m[OFFSET:], lw=1, label=f"{FILENAME}")
    x = [(i+OFFSET) for i in x]
